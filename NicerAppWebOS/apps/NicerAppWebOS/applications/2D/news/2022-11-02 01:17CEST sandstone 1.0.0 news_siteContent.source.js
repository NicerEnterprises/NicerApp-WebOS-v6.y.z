				...
				onload : function (settings) {
                    ...
                    na.m.waitForCondition ('/NicerAppWebOS/apps/NicerAppWebOS/applications/2D/news start', na.m.HTMLidle, function () {
                        ...
                        na.m.waitForCondition('news app : siteContent dialog reappearance', function () {
                            ...
                            loadedIn.settings.initialized = true;
                            loadedIn.settings.ready = true;
                            document.addEventListener ('keyup', na1.onkeyup);
                            
                            na1.startApp();
                            if (typeof settings.callback=='function') settings.callback('siteContent');
                            na.analytics.logMetaEvent ('/NicerAppWebOS/apps/NicerAppWebOS/applications/2D/news (version '+na1.about.version+' is starting.');
...
debugger;
        if (unread < 100) {
            c.dtEnd = c.dtCurrent;
            c.dtCurrent = new Date(c.dtEnd.getTime() - (1000 * 60 * 60 * g.readHistory_numHours));//c.read_loop_minutesIntoPast));
...
debugger;
    ...
    displayNewNewsItems : function () {
        ...
        /*
        //alert (g.numItemsThatFitScreen);
        var
        min2 = g.numItemsThatFitScreen * 0.1,
        max2 = g.numItemsThatFitScreen * 0.2,
        min = g.numItemsThatFitScreen - max2,
        max = g.numItemsThatFitScreen - min2,
        */
...
debugger;
        ...
        /*
        clearTimeout (c.timeout_postDisplayNewNewsItems);
        c.timeout_postDisplayNewNewsItems = setTimeout (function() {
            na1.loadNews_read_loop();
            //na1.onresize();
            //na1.countDown();
        }, 2000);
    ...
    displayNewNewsItem : function () {
        ...
        //if (c.timerDisplayItem) clearTimeout (c.timerDisplayNewsItem);
        /*c.displayNewsItems = *///setTimeout (function() {
            ...
            ks = ks.sort(function(a,b){ b - a }),
            unread = dc[ks[0]];
            //console.log ('t33:'+c.tries+':'+unread);
            //console.trace();

            if (!g.searchQuery) {
                if (unread < 100) { //g.numItemsThatFitScreen * 1.5) {
                    na1.loadNews_read_loop();
                    //return false;
                }
            } else {
                if (unread < 100) { //g.numItemsThatFitScreen) {
                    c.dtEnd = c.dtCurrent;
                    c.dtCurrent = new Date(c.dtEnd.getTime() - (1000 * 60 * c.read_loop_minutesIntoPast));
            ...
            if (found || !na1.match_searchCriteria(it)) {
                ...
                $('#newsApp_content').append(html).delay(50);
 ...
 *                var
                //min = 500,
                //max = 700,
                min = ($('#siteContent__content').width()-100)/3,
                max = ($('#siteContent__content').width()-100)/2,
                //min = 450,
                //max = 550,
                ...
                //min = 350,
                //max = 700,
                min = ($('#siteContent__content').height()-100)/4,
                max = ($('#siteContent__content').height()-100)/2,
                //min = 350,
                //max = 450,
                ...
                $(dnfi.id).css({position:'relative',minWidth:'',width:'auto',maxWidth:w,minHeight:'',height:'auto',maxHeight:h}).animate({opacity:1},'slow');
*/
                ...
                $('.vividScrollpane', el).add($('.newsApp__item__mediaSingle img', el)).css ({
                    width : '40%',
                    /*height :
                        $(el).height()
                        - ( $('.newsApp__item__title')[0]?$('.newsApp__item__title').outerHeight():0 )
                        - ( $('.newsApp__item__footer')[0]?$('.newsApp__item__footer').outerHeight():0 )*/
                });
                else
                $('.vividScrollpane', el).css ({
                    width : w
                });


                //$(dnfi.id+'__bg').css({width:w,height:w}).delay(50);
                ...
                c.dnf = dnf;
                                        ...
                                        //console.log (itEl.id+':'+sph1);
    h -= sph1;


    /*
                $(dnfi.id).css ({
                    minWidth : '', width : w, maxWidth : w,
                    minHeight : '', height : h, maxHeight : h
                });
    */
                ...
                //if (c.tries === g.numItemsThatFitScreen * 1.5) { //g.numItemsThatFitScreen) {
                if (full || removed) {
                    debugger;
                ...
                //if (c.tries > g.numItemsThatFitScreen) return true;
    ...
    //                            width : sp.offsetWidth - 30,
                            width : $(el).parent('div').width()//$(itEl).width() - jel2.width() - $(el).position().left - 50,
                            //height : ''
                        });
    ...
    debugger;
                ...
                if (prefix=='') {
                    ...
                    if (!na.apps.loaded['/NicerAppWebOS/apps/NicerAppWebOS/applications/2D/news']) return false;
                        ...
                        $('.newsApp__item__outer', $('#newsApp_content')[0]).css({height:'',maxHeight:450,opacity:1});
                if (prefix!=='' && full) {
                    na1.countDown();
    ...
    */
    ...
    displayNews_formatItem : function (it, prefix) {
        ...
        html += '<div id="newsApp__item__'+it.idx+'" class="newsApp__item__outer" style="width:'+w+'px;height:fit-content;'+fontSize+';opacity:0.00001">';
        ...
        if (typeof it.de=='string') html+= '<div class="newsApp__item__contentContainer"><div class="newsApp__item__mediaSingle"></div><div id="newsApp_item_'+it.idx+'__scrollpane" class="newsApp__item__desc vividScrollpane" style="width:50px;height:50px;">' + it.de.replace(/\&#39;/g, '\'').replace(/#39;/g, '\'') + '</div></td></tr></table><div style="height:1px;overflow:hidden;">&nbsp;</div></div>';
    ...
    formatDate : function (it, noPubDate, no2ndLine) {
        ...
        r3 = '<span class="newsApp__item__date__local">Local : ' + m.getFullYear() + "-" +
            ("0" + (m.getMonth()+1)).slice(-2) + "-" +
            ("0" + m.getDate()).slice(-2) + "(" + dns[m.getDay()] + ') ' +
            ("0" + m.getHours()).slice(-2) + " : " +
            ("0" + m.getMinutes()).slice(-2) + " : " +
        ...
        */
        r2 = '<span class="newsApp__item__date__local">Local : ' + na1.formatDateTime(m1) + '</span>',
        r1 = '<span class="newsApp__item__date__remote">Remote : ' + ( it.pubDate ? it.pubDate : na1.formatDateTime(m) ) + '</span>',
    ...
    onresize_do : function (sel) {
        ...
        $els2.css({position:'absolute',opacity:0.0001}).delay(50).each(function(idx,el) {
            $('.vividScrollpane',el).css({float:'left',position:'absolute',opacity:0.0001}).delay(50).each(function(idx2,el2) {
                ...
                $els2.add(el2).css({position:'relative'}).animate({opacity:1},'slow').delay(50);
...
//                $('.newsApp__item__contentContainer',el).css({display:'none'});
                ...
                //setTimeout (function(el) {
                  //  $('#newsApp__item__contentContainer',el).css({display:'flex'});
                    ...
                    min = ($('#siteContent__content').width()-100)/(
                        $(window).width() < na.site.globals.reallySmallDeviceWidth
                        || $(window).width() < na.site.globals.reallySmallDeviceWidth
                        ? 1
                        : $(window).width() > 2000
                    ...
                    max = 1.3,
                    mp1 = Math.random() * (max - min + 1) + min,
                    min = ($('#siteContent__content').height()-100)/ (
                        $(window).width() < na.site.globals.reallySmallDeviceWidth
                        || $(window).width() < na.site.globals.reallySmallDeviceWidth
                        ? 1
                        : $(window).width() > 2000
                    ...
                    //console.log (el.id+':'+sph);
                    //console.log (el.id+'::'+th);
                    ...
                    //smartphone fix.
                    if (spw<310 && $('.newsApp__item__mediaSingle', el)[0] && $('.newsApp__item__mediaSingle', el).width() > 0)
                        spw = 310 - $('.newsApp__item__mediaSingle', el).width();

                    spw -= 10;
                    spw = $(el).width()-10;
                    //alert (el.id+': '+spw+' - ' + sph);
                    $(el2)
                        .css({position:'relative',padding:'5px !important',width:spw-10,opacity:1,height:'auto',maxHeight : 'fit-content' });
                        //.css ({position:'relative',flexGrow:1,flexShrink:1,margin:5,marginRight:10,opacity:1})
                    $(el2).parents('.newsApp__item__contentContainer')
                        .addClass('vividScrollpane')
                        .css({position:'relative',opacity:1,padding:'5px !important',width:spw+20,opacity:1,height:'auto',maxHeight : 'fit-content'})
                        .delay(300);

                    /*
                    var elh = 0;
                    if ($('.newsApp__item__title', el)[0]) {
                        elh += $('.newsApp__item__title', el).outerHeight();
                        var mh = $('.newsApp__item__title', el).css('marginTop');
                        if (typeof mh=='number') elh += parseInt(mh);
                        var mh = $('.newsApp__item__title', el).css('marginBottom');
                        if (typeof mh=='number') elh += parseInt(mh);
                        if ($(el2).html()==='') elh += 10; else elh += 10;
                    }
                    if ($('.newsApp__item__contentContainer', el)[0]) var elh2 = $('.newsApp__item__contentContainer', el).outerHeight();
                    if (elh2===0) elh2 = th / 2.5 - $('.newsApp__item__title').outerHeight() - $('.newsApp__item__footer').outerHeight();
                        var mh = $('.newsApp__item__contentContainer', el).css('marginTop');
                        if (typeof mh=='number') elh += parseInt(mh);
                        var mh = $('.newsApp__item__contentContainer', el).css('marginBottom');
                        if (typeof mh=='number') elh += parseInt(mh);

                    elh += elh2;
                    if ($('.newsApp__item__footer', el)[0]) {
                        elh += $('.newsApp__item__footer', el).outerHeight();
                        var mh = $('.newsApp__item__footer', el).css('marginTop');
                        if (typeof mh=='number') elh += parseInt(mh);
                        var mh = $('.newsApp__item__footer', el).css('marginBottom');
                        if (typeof mh=='number') elh += parseInt(mh);
                    }
                    if (typeof $(el2).css('marginBottom')=='number') elh+= parseInt($(el2).css('marginBottom'));
                    //if ($(el2).html()==='') elh += 10; else elh += 10;
                    elh += 15;
                    //alert (elh);

                    console.log (el.id+':::'+spw);
                    console.log (el.id+':::'+elh);
                    if ($(window).width() < na.site.globals.reallySmallDeviceWidth) var mw = $('#newsApp_content').width();
                    else var mw = spw;
                    */
