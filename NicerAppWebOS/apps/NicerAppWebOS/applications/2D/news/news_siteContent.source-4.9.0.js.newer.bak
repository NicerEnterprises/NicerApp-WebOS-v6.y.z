delete na.apps.loaded['applications/2D/news'];
na.apps.loaded['applications/2D/news'] = {
	about : {
	...
	settings : {
		loadedIn : {
			'#siteContent' : {
                ...
                mergeMenu : function () {
                        var
                        na1 = na.apps.loaded['applications/2D/news'], g = na1.globals, s = na1.settings, c = s.current, db = c.db,
                        loadedIn = s.loadedIn['#siteContent'],
                        mainmenu = $('#newsApp_mainmenu')[0];
                        //na.apps.loaded['applications/2D/news'].transformLinks ($('#newsApp_mainmenu')[0]);
				...
				onload : function (settings) {
                    ...
                    na.m.waitForCondition ('applications/2D/news start', na.m.HTMLidle, function () {
                        var
                        na1 = na.apps.loaded['applications/2D/news'], g = na1.globals, s = na1.settings, c = s.current, db = c.db,
                        loadedIn = s.loadedIn['#siteContent'];
                        ...
                        na.site.settings.current.app = 'applications/2D/news';
                        ...
                        na.m.waitForCondition('siteContent dialog reappearance', function () {
                            ...
                            na.apps.loaded['applications/2D/news'].settings.loadedIn['#siteContent'].settings.initialized = true;
                            na.apps.loaded['applications/2D/news'].settings.loadedIn['#siteContent'].settings.ready = true;
                            document.addEventListener ('keyup', na.apps.loaded['applications/2D/news'].onkeyup);
                            
                            na.apps.loaded['applications/2D/news'].nestedStartApp();
                            na.analytics.logMetaEvent ('applications/2D/news (version '+na.apps.loaded['applications/2D/news'].about.version+' is starting.');
                ...
                ondestroy : function (settings) {
                    ...
                    document.removeEventListener ('keyup', na.apps.loaded['applications/2D/news'].onkeyup);
                    $(na.apps.loaded['applications/2D/news'].settings.loaderIcon).remove();
                },
				onresize : function (settings) {
					// TODO : what's settings.isManualResize ???
					//if ($('#appGame').css('display')=='none') $('#appGame').fadeIn('slow');
					na.apps.loaded['applications/2D/news'].onresize(settings);
	...
	nestedStartApp : function () {
        ...
        }, na.apps.loaded['applications/2D/news'].startApp, 20);
	},
    ...
    startApp : function () {
        ...
        na.analytics.logMetaEvent ('start app : applications/2D/news.v'+na.apps.loaded['applications/2D/news'].about.version);
        ...
        na.apps.loaded['applications/2D/news'].settings.loaderIcon = na.acs.addIcon(
        ...
        c.dtCurrent = new Date(new Date().getTime() - (1000 * 60 * 60 * g.readHistory_numHours )); // read back 1 hour initially
        c.dtEnd = new Date();
        ...
        if (!c.read_loop_minutesIntoPast) c.read_loop_minutesIntoPast = 60 * g.readHistory_numHours;
    ...
    onkeyup : function (evt) {
        //debugger;
        if (evt.code=='AltRight' || evt.code=='AltLeft') na.apps.loaded['applications/2D/news'].gotoNextPage();
        if (evt.key==' ') na.apps.loaded['applications/2D/news'].toggleLock();
    },
    
    viewSearchbar : function (evt) {
        if ($('#div_newsApp_searchbar').css('display')=='block') {
            na.apps.loaded['applications/2D/news'].clearSearch(evt);
        } else {
    ...
    onSearch : function (evt) {
        ...
        c.dtCurrent = new Date(new Date().getTime() - (1000 * 60 * 60 * g.readHistory_numHours));
        c.dtEnd = new Date();
        c.lastCurrentGet = new Date();
        c.locked = false;
        c.firstRun = true;
        ...
        c.loads = 0;
        c.failedLoads = 0;
        na.apps.loaded['applications/2D/news'].settings.current.db = [];
        ...
        na.m.waitForCondition ('news app not loading?', function () {
            ...
            ajaxCommand = {
                ...
                success : function (data, textStatus, jqXHR) {
                    ...
                    na.m.extend (na.apps.loaded['applications/2D/news'].settings.current.db, data);
                    ...
                    na.apps.loaded['applications/2D/news'].settings.current.db = na.apps.loaded['applications/2D/news'].settings.current.db.concat(data);
                    ...
                    if (data.length === 0) {
                        ...
                        c.displayCounts = '<span class="newsApp__header__displayCounts">' + c.displayCounts + '</span>';
                        ...
                        var
                        dnf = na.apps.loaded['applications/2D/news'].displayNews_full(),
                        full = dnf.full,
                        removed = dnf.removed;
    ...
    */
    ...
    loadNews_get_forDateTimeRange : function (dtBegin, dtEnd, settings) {
        ...
        na.m.waitForCondition ('news app not loading (c.loads==='+c.loads+')?', function () {
            var r = !s.loading;
            return r;
        }, function() {
            s.loading = true;
            ...
            ajaxCommand = {
                ...
                success : function (data, textStatus, jqXHR) {
                    ...
                    //na.m.log (1, 'loadNews_get_forDateTimeRange() : ' + data);
                    ...
                    var
                    db = na.apps.loaded['applications/2D/news'].settings.current.db,
                    dc = {},
                    di = {},
                    params = {
                        dc : dc,
                    ...
                    var
                    dbg = {
                        appID : 'applications/2D/news (version '+na.apps.loaded['applications/2D/news'].about.version+')',
                        loads : c.loads,
                        section : settings.section.replace(/-/g,'/').replace(/ /g, '_'),
                        numLoaded : c.unread,
                        dateBegin : dtBegin,
    ...
    displayNews_loop : function (state) {
        ...
        if (found || !na1.match_searchCriteria(it)) {
            ...
            //if (state=='testing') state = 'displaying';
            c.state = state;
            ...
            if (c.tries === getToTry) {
                //debugger;
                na.apps.loaded['applications/2D/news'].settings.dnf.full = true;
                na1.countDown();
                return true;
            }
            ...
            function displayNewsItem(state) {
                if (!na.apps.loaded['applications/2D/news']) return false;
...
//                            width : sp.offsetWidth - 30,
                ...
                c.timerDisplayNewsItem = setTimeout (function (it, prefix, state) {
                    ...
                    imgs.each(function(idx,el){
                        if ($(this).width() > na.apps.loaded['applications/2D/news'].settings.itemWidth - 10) {
                            $(this).css ({
                                width : na.apps.loaded['applications/2D/news'].settings.itemWidth - 10,
                                height : ''
                            })
                            el.removeAttribute('height');
                            el.removeAttribute('width');
                    ...
                    c.timerCheck = setTimeout (function(itEl, prefix, state) {
                        ...
                        if (prefix=='') {
                            na.apps.loaded['applications/2D/news'].settings.dnf = dnf;
    ...
    countDown : function () {
        ...
        //debugger;
        if (!na.apps.loaded['applications/2D/news'].settings.dnf.full) return false;
        
        $('.newsApp__item__title > a, .newsApp__item__desc a').not(':has(img)').animate({opacity:0.7});
    ...
    displayNews_formatItem : function (it, prefix) {
        ...
        if (typeof it.de=='string') {
            ...
            it.de = it.de.replace(/<img/g, '<img onload="var na1 = na.apps.loaded[\'applications/2D/news\'], g = na1.globals, s = na1.settings, c = s.current;  var ni = $(this).parents(\'.newsApp__item__outer:first\');  if (ni[0]) { var dnf = na1.displayNews_full(\''+prefix+'\'), id = ni[0].id, resize = c.resize[id];  if (prefix==\'\') na.apps.loaded[\'applications/2D/news\'].settings.dnf = dnf; if (!dnf.removed && typeof resize==\'function\') setTimeout(function() { resize(this,event,'+it.idx+',\''+prefix+'\'); }, 100); if (dnf.removed) { ni.remove(); }; if (dnf.removed && c.tries < g.onScreen + 1000) na1.displayNews_loop(); /*else { if (dnf.removed) na1.countDown(); }*/}"');
        }
        ...
        if (typeof it.de=='string') {
            if (typeof it.t=='string' && it.t!='' && it.de.indexOf(it.t)===-1) html+= '<span class="newsApp__item__title"><a class="nomod" target="newsAppItem_'+it.idx+'" href="' + it._id+'">' + it.t.replace(/\&#39;/g, '\'').replace(/#39;/g, '\'')+ '</a></span><br/>';
        } else {
            if (typeof it.t=='string' && it.t!='') html+= '<span class="newsApp__item__title"><a class="nomod" target="newsAppItem_'+it.idx+'" href="' + it._id+'">' + it.t.replace(/\&#39;/g, '\'').replace(/#39;/g, '\'') + '</a></span><br/>';
        }
        ...
        html += '<span class="newsApp__item__date"><a class="nomod" target="newsAppItem_'+it.idx+'" href="' + it.u+'">' + na.apps.loaded['applications/2D/news'].formatDate(it)+'</a></span><br/>';
        ...
        html2+= '<a class="nomod" target="newsAppItem_'+it.idx+'_rssURL" href="' + it.rssURL+'">'+na.apps.loaded['applications/2D/news'].urlToDomainName(it.rssURL)+'</a></span>';
        html2+= '<br/>';
        ...
        var pd = na.apps.loaded['applications/2D/news'].formatDate(it);
	...
	getURLparameters : function () {
        ...
        x = na.site.globals.app[prefix+'applications/2D/news'];
        //debugger;
        return [
    ...
    onresize : function (settings, displayNews) {
        ...
        var 
        $els2 = $('.newsApp__item__outer, .newsApp__item__outer > table'),
        $els3 = $('#newsApp_content > .newsApp__item__outer, #newsApp_content > .newsApp__item__outer > table');
        $('#newsApp_content_shadow').css({ overflow : 'hidden', opacity : 0.00001 });
        debugger;
        na.m.waitForCondition ('applications/2D/news.onresize() : na.m.HTMLidle()?', na.m.HTMLidle, function () {
            debugger;
            ...
            na.apps.loaded['applications/2D/news'].settings.itemWidth = w;

            $els2.css({height:'',width:w, display : 'inline-block'}).fadeIn('fast');
            $els3.css({height:'',width:w, display : 'inline-block', opacity:1});
            
            
            if (displayNews && c.dtCurrent) 
                na.m.waitForCondition(
                    'applications/2D/news::onresize() : have news items to display?',
                    function () {
                        var r = c.unread > 0;
                        return r;
                    },
