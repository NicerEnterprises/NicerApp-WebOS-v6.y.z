seductiveapps.menu = seductiveapps.vividMenu = {
	about : {
		...
		copyright : '(c) (r) 2011-2016 by [the owner of seductiveapps.com] <seductiveapps@gmail.com>',
	...
	init : function (cihe, initID) {
		if (
			...
			if (sa.menu.globals.logging) sa.m.log (2, { msg : 'sa.vividMenu.init(): for initID='+initID+'; NOT initializing element with id="'+cihe.id+'" and class="'+cihe.className+'", because .style.display=="none".' } );
			sa.vcc.componentFullyInitialized (initID, cihe.id);
			return false;
		} else {
			if (sa.menu.globals.logging) sa.m.log (2, { msg : 'sa.vividMenu.init(): for initID='+initID+'; Initializing element with id="'+cihe.id+'" and class="'+cihe.className+'".' } );
		};
		sa.menu.loadMenuGraphics (cihe, initID);
	},
	
	initMenu : function (componentInstanceHTMLid) {
		if (sa.s.c.settings.mayDisplayMenus) jQuery('#'+componentInstanceHTMLid).css({opacity:0.001});
		sa.menu.preprocess (componentInstanceHTMLid);
		...
		//debugger;
		sa.menu.getitems (document.getElementById(componentInstanceHTMLid));
		...
		//if (sa.menu.globals.logging) sa.m.log (1, 't10', level);
		//if (sa.menu.globals.logging) sa.m.log (1, 't11', html);
	...
	onresize : function (componentInstanceHTMLid) {
		...
		if (!sa.vcc.settings[componentInstanceHTMLid]) {
			if (sa.menu.globals.logging) sa.m.log (1, 'sa.menu.onresize called with invalid componentInstanceHTMLid "'+componentInstanceHTMLid+'"');
			return false;
		}
		...
		if (componentInstanceHTMLid=='musicMenu') {
			...
			css = { 
				//width : p.left + furthestAway.x + w,
				height : p.top + furthestAway.y + h + 32
			},
			...
			if (el1 && el2) {
				...
				if (sa.menu.globals.logging) sa.m.log (1, 'sa.menu.onresize() : el1 or el2 was not available - afterResize({resized:false});!');
				resized = false;
			}
		};
	...
	preprocess : function (componentInstanceHTMLid) {
		...
		if (
		  !s.originalContent
		  && jQuery('#'+componentInstanceHTMLid)[0]
		  && jQuery('#'+componentInstanceHTMLid)[0].children[0]
		) {
			s.originalContent = jQuery('#'+componentInstanceHTMLid)[0].children[0].innerHTML;
		}
	...
	merge : function (componentInstanceHTMLid, transformLinks, valueToFind, otherMenu, iframeHTMLid, targetIsInsideIframe) {
		...
		/*
		if (sa.menu.globals.logging) sa.m.log (1, 't31', s, lp);
		if (sa.menu.globals.logging) sa.m.log (1, 't32 menuContentFixed', menuContentFixed);
		if (sa.menu.globals.logging) sa.m.log (1, 't33 html', html);
		debugger;
		*/
	...
	// called by sa.vcc.init()
		...
		if (sa.menu.globals.logging) sa.m.log (undefined, { sample : 1, s : s } );
		//if (typeof s.items!=='object' || s.items.length===0) {
			s.items = [];
			if (cihe.children[0]) {
			  sa.menu.getitemsForUL (cihe, cihe.children[0], '', 0, null);	
			  if (sa.menu.globals.logging) sa.m.log (undefined, { sample : 2, s : s } );
			}
		//}
		if (cihe.children[0]) {
	...
	getitemsForLI : function (cihe, el, idx, path, level, parent) {
		...
		item.dontKeepSelected = el.children[0].className.indexOf('menu__dontKeepSelected')!==-1;
			//if (sa.menu.globals.logging) sa.m.log (1, 't35', href, sa.m.globals.urls.app + '#');
					...
					//debugger;
					if (
						newURL.match('javascript:')
						&& !newURL.match('javascript:/')
					) {
						n = newURL;
					} else {
						n = 'javascript:'+p+'sa.s.c.pushState (null, "' + this.title + '", "' + newURL + '")';
					} 
				
					item.url = n; // c.children[0].href.replace("'", "\\'");
					//s.buttonText = item.label;
					jQuery(el).attr('url', item.url);
		...
		*/
		...
		//if (sa.menu.globals.logging) sa.m.log (undefined, { s : s, item : item } );
	...
	loadMenuGraphics : function (cihe, initID) {
		//if (sa.menu.globals.logging) sa.m.log (6, 'vividMenu.loadMenuGraphics 1', cihe, initID, sa.vcc.settings);
		if (sa.vcc.newInstanceNeeded(cihe)) sa.vcc.newInstance (cihe, initID);
		var s = sa.vcc.settings[cihe.id];
		//if (sa.menu.globals.logging) sa.m.log (6, 'vividMenu.loadMenuGraphics 2', cihe, initID, s, sa.vcc.settings);
		var t = sa.vcc.getVividTheme (s.themeName); 
                if (!t) return false;
		s.theme = t;
		//s = sa.vcc.newInstanceThemeOverrides (s, t);
		//sa.menu.getitems (cihe);
		//if (sa.menu.globals.logging) sa.m.log (6, 'vividMenu.loadMenuGraphics 3', cihe, initID, s, sa.vcc.settings);
		
		var img = document.createElement('img');
                //debugger;
		img.src = s.theme.url;//.replace('vividTheme__','');
...
//debugger;
		...
		if (doClick) {
			...
			if (url != "''") {
				if (url.match(/javascript:/) && url.match ('sa.apps')) {
					var
					p1 = url.indexOf ('sa.apps'),
					p2 = url.indexOf (';', p1),
					js = url.substr(p1, p2-p1),
					js = 'window.top.'+js;
					
					//debugger;
					eval(js);
				} else if (url.replace(/javascript:/g, '').replace(/;"/,'')) {
					var 
					js = 
						url.replace(sa.m.globals.urls.app, '')
					eval(js);
				} else {
	...
	onmouseover : function (el,evt) {
		...
		clearTimeout (sa.sb.settings.current.timeoutForMessageClearing);
		sa.statusbar.update (item.url, true);

		if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): onmouseover ' + idx + '('+s.items[idx].label+')');
		...
		if (
			...
			if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover: cancelled s.immediateChildHidingTimeout');
		};
...
//		}, 300);
		...
		*/
		...
		if (s.menuOpens[idx]) {
			if (s.menuOpens[idx].timeout) {
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): cancelling[1] close of menu '+idx+'('+s.items[idx].label+')');
		...
		for (var idx2 in s.menuOpens) {
			if (s.items[idx].path.indexOf (s.items[idx2].id)!==-1) {
				if (s.menuOpens[idx2].timeout) {
					if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): cancelling[2] close of menu '+idx2+ '('+s.items[idx2].label+')');
		...
		for (var i=idx+1; i<s.items.length && s.items[i] && s.items[i].level > initialLevel; i++) {
				//if (sa.menu.globals.logging) sa.m.log (1, 't13', s.items[i].path, s.items[idx].path+','+idx);
		...
		if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover() : setting/replacing timeout on s.menuOpenTimeout for childLabels=="'+childLabels+'".');
		if (s.menuOpenTimeout) clearTimeout(s.menuOpenTimeout);
		
		s.menuOpenTimeout = setTimeout(function() {
			if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover() : fading in (s.menuOpenTimeout) childLabels=="'+childLabels+'".');
			if (childIDs!='') {
				if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): opening : '+idx+', '+childIDs);
			...
			//if (s.menuTooSmall) {
				for (var elID in s.menuOpensTemp) {
					if (s.menuOpensTemp[elID]) {
						if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover() : clearing s.menuOpensTemp for '+elID+' ('+s.items[elID].label+'), s.menuOpensTemp["'+elID+'"]==="'+s.menuOpensTemp[elID]+'".');
				...
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): ps = '+s.items[idx].path);
				
				for (var idx5 = 0; idx5 < ps.length - 1; idx5++) {
					var 
					elID = parseInt(ps[idx5]);
					
					if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): clearing s.menuOpensTemp for '+elID+' ('+s.items[elID].label+')');
					if (s.menuOpensTemp[elID]) clearTimeout(s.menuOpensTemp[elID]);
					if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseover(): s.menuOpensTemp set on '+elID+' ('+s.items[elID].label+')');
	...
	onmouseout : function (el,evt) {
		...
		clearTimeout (sa.sb.settings.current.timeoutForMessageClearing);
		sa.sb.settings.current.timeoutForMessageClearing = setTimeout (function () {
			sa.statusbar.update (sa.sb.settings.current.defaultMsg, true);
		}, 1000);

		if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): onmouseout ' + idx + '('+s.items[idx].label+')');
		
		clearTimeout (s.menuOpenTimeout);
				...
				if (mot == 'done') {
					...
					if (!sa.menu.globals.keepAllSubMenusOpen) {
						s.showNodeTimeout = setTimeout (function () {
							...
							if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): showing node '+idx3+' ('+s.items[idx3].label+'), because s.menuOpensTemp[idx3]==="done"');
		...
		if (
			...
			&& (
				...
				clearTimeout(s.immediateChildHidingTimeout);
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout: cancelled s.immediateChildHidingTimeout');
			
		}
		
		
		if (sa.menu.globals.logging) sa.m.log (1, 't555', 's.previousIdx='+s.previousIdx+', idx='+idx+', s.items[idx].label='+s.items[idx].label);
		if (
			s.items[idx]
			...
			if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): s.immediateChildHidingTimeout to hide set[2] on '+idx+' ('+s.items[idx].label+')');
			//debugger;
		...
		//if (s.menuTooSmall) {
			...
			if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): ps = '+s.items[idx].path);
			...
			for (var idx5 = 0; idx5 <= ps.length - 1; idx5++) { 
				...
				if (s.menuOpensTemp[elID]) {
					clearTimeout(s.menuOpensTemp[elID]);
					if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): clearing s.menuOpensTemp for '+elID+' ('+s.items[elID].label+')');
				};

				if (s.items[elID]) {
					if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): s.menuOpensTemp set for hideNode on '+elID+' ('+s.items[elID].label+')');
					s.menuOpensTemp[elID] = setTimeout(
						'sa.menu.hideNode("'+menuHTMLid+'", "'+elID+'", true);'
						,menuWalkbackLapse);
			...
			if (ps.length>=3) {
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): clearing s.menuOpensTemp for '+elID+' ('+s.items[elID].label+')');
				if (s.menuOpensTemp[elID]) clearTimeout(s.menuOpensTemp[elID]);
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.onmouseout(): s.menuOpensTemp set for showNode on '+elID+' ('+s.items[elID].label+')');
				s.menuOpensTemp[elID] = setTimeout(
	...
	startWalkback : function (menuHTMLid, s, idx) {
		...
		if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.startWalkback(): s.walkbackIdxTemp='+s.walkbackIdxTemp+', s.walkbackIdx='+s.walkbackIdx);
		...
		if (s.walkbackIdx >= 0) {	
			...
			if (elID) {
				elID = parseInt(elID);
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.startWalkback(): calling hideNode for '+elID+' ('+s.items[elID].label+')');
				setTimeout (function () {
					sa.menu.hideNode(menuHTMLid, elID, true);
		...
		if (s.walkbackIdxTemp >= 0) {
			...
			if (elID) {
				elID = parseInt(elID);
				if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.startWalkback(): calling showNodeTemp for '+elID+' ('+s.items[elID].label+')');
				//setTimeout (function () {
		...
		if (s.walkbackIdx >= 0 || s.walkbackIdxTemp >= 0) {
			if (sa.menu.globals.debugMouseOpeningAndClosing) if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.startWalkback(): setting s.walkbackTimeout');
			s.walkbackTimeout = setTimeout(function() {
				sa.menu.startWalkback (menuHTMLid, s, idx);
			}, sa.menu.globals.menuWalkbackLapse);
		}
	...
	instaHide : function (menuHTMLid) {
		...
		if (s.menuOpens) {
			for (var idx in s.menuOpens) {
				...
				if (ids!='') {
					if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.hideNode(): ' +idx+ ' --> '+ idsLabels);
	...
	showNode : function (menuHTMLid, idx, del) {
		...
		if (s.menuOpens[idx]) {
			...
			if (ids!='') {
				if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.showNode(): ' +idx+ ' --> '+ idsLabels);
				if (sa.m.userDevice.isPhone) {
	...
	showNodeTemp : function (menuHTMLid, idx, del) {
		...
		if (s.menuOpensTemp[idx]) {
			...
			//debugger;
			if (childIDs!='') {
				if (sa.menu.globals.logging) sa.m.log (3, 'sa.vividMenu.showNodeTemp(): ' +idx+ ' --> '+ childLabels);
				if (sa.m.userDevice.isPhone) {
