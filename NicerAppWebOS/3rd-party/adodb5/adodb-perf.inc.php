...
<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
	...
	*/
	...
	$view = $conn->host;
	if ($conn->host && $conn->database) $view .= ', db=';
	$view .= $conn->database;
	...
	echo "<title>ADOdb Performance Monitor on $view</title><body bgcolor=white>";
	...
	global $ADODB_PERF_MIN;
	$view .= " (Min sql timing \$ADODB_PERF_MIN=$ADODB_PERF_MIN secs)";

	if  (empty($_GET['hidem']))
	echo "<table border=1 width=100% bgcolor=lightyellow><tr><td colspan=2>
	<b><a href=https://adodb.org/dokuwiki/doku.php?id=v5:performance:performance_index>ADOdb</a> Performance Monitor</b> <font size=1>for $view</font></tr><tr><td>
