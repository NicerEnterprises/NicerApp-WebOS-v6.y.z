      ...
      */
      //if (!is_null($_SESSION['na_error_log_filepath_html'])) {
        ...
        $dbgHTML =
          ...
          .'</div>';
        */
        if (array_key_exists('dbgNum2',$_SESSION)) $_SESSION['dbgNum2']++;
      ...
      if (!array_key_exists('startedID', $_SESSION))
        $_SESSION['startedID'] = randomString(50);

      $err = [
          'type' => 'CouchDB query',
          's1' => array_key_exists('started', $_SESSION)
              ? $_SESSION['started']
              : time()//microtime(true)
          ,
