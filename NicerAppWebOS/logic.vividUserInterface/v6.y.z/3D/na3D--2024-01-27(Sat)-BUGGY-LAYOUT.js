                    ...
                    //debugger;    
                        ...
                        for (var j=0; j<t.items.length; j++) {
                            ...
                            if (
                                ...
                                && hoveredItem.it.idxPath === child.parent.idxPath
                            ) {
                                var
    ...
    initializeItems_walkKey (cd) {
        ...
        if (ps[ps.length-1]=='files') {
            ...
            */
            ...
            var pk = cd.path;//+'/'+cd.k;
            if (!cd.params.ld2[pk]) {
                var pk2 = cd.path.replace(/\/folders$/,'');
                cd.params.ld2[pk] = {
                    levelIdx : 0,
                    idx : (cd.params.ld2[pk2] ? cd.params.ld2[pk2].idx : 0)
                };
                newLevel = true;
            }
            ...
            //debugger;
            if (cd.k.match(/1970s/)) debugger;
            if (cd.k.match(/Creedence/)) debugger;
            if (cd.k.match(/21st/)) debugger;
            if (cd.k.match(/\(Hard-/)) debugger;

            if (!cd.k.match(/\.mp3$/)) {
                while (lp && (!started || lp!==lastParent)) {
                    //if (!started && cd.level>4) lastParent = lastParent.parent;
                    if (idxp!=='') idxp = '/' + idxp;
                    idxp = lp.idx + idxp;
                    lp = lp.parent;
                    started = true;
                ...
                if (cd.k.match(/Suzanne/)) debugger;
                idxp = cd.params.idxPath;
            }

            if (!cd.k.match(/\.mp3$/))
                cd.params.idxPath = idxp;
            if (cd.k.match(/\(Hard-/)) debugger;
            if (cd.level >= 6) {
                idxp = cd.params.idxPath.replace(/\/[\d]+$/,'');
                if (idxp=='') idxp = '/0';
            } else {
                idxp = cd.params.idxPath;//+'/'+(cd.params.t.items.length);
                if (idxp=='') idxp = '/0';
            }

            if (idxp.match(/\/1$/)) debugger;
            ...
            if (newLevel) {
                debugger;
                cd.params.idxPath += '/' + (cd.params.t.items.length-1);
                cd.params.ld2[pk].levelIdx++;
                cd.params.ld2[pk].idx = cd.params.t.items.length;
            };
...
//if (it.name.match(/early 21st/i)) debugger;
            if (it.model) {
                if (it.name.match(/\.mp3$/)) {
                    ...
                    it.model.position.x = Math.round(
                        p.model.position.x
                        + (/*u2*/it.columnField * 500)//+(it.columnField*m1)
                        + (p.columnField * 1500)
                        //+ rndx
                    ...
                    it.model.position.y = Math.round(
                        p.model.position.y
                        + (it.rowField /* * v2*/ * 500)//+(it.rowField*m2)
                        + (p.rowField * 1500)
                        //+ rndy
