...
export class na3D_fileBrowser {
    constructor(el, parent, parameters) {
        ...
        t.flyControls.movementSpeed = 300;
        t.flyControls.dragToLook = false;
        t.flyControls.rollSpeed = 0.01;//Math.PI / 24;
    ...
    animate(t, evt) {
        ...
        //if (t.mouse.x!==0 || t.mouse.y!==0) {
            ...
            if (false && t.debug) console.log ('t.flyControls', t.flyControls.enabled, 't.cameraControls', t.cameraControls.enabled, 't.orbitControls', t.orbitControls.enabled);
            ...
            if (false && t.cameraControls.enabled) {
                ...
                if (t.flyControls.enabled) {
                    ...
                    if (false) {
                        ...
                        //tar.applyQuaternion(t.camera.quaternion).add(t.flyControls.object.position);//t.camera.position); // won't work in either case
                        tar.set(0,0,0).applyQuaternion(t.camera.quaternion).add(t.camera.position); // ALSO won't work in either case
            ...
            if (false) {
                var dbg = {
                        ...
                        'btns' : t.mouse.event ? t.mouse.event.buttons : null,
                        't.lookClock' : t.lookClock
                    };
                    //debugger;
                //if (t.debug) console.log (dbg);
                ...
                if (it) {
                    ...
                    if (it.parent) {
                        ...
                        if (it.name.match(/\.mp3$/)) {
                            ...
                            var cube = t.createSphere (t.meshLength, it.color);
                        }
...
export class na3D_demo_models {
    constructor(el, parent, data) {
        ...
        $(el).bind('pointerup', function() { t.onPointerUp (event, t) });
