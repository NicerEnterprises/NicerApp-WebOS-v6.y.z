...
class naVividButton_icon {
    ...
    increaseGradient(t) {
        ...
        else if (t.elText) t.setTextElementColor(t);
    }
    
    decreaseGradient(t) {
        t.gradientRadius -= 2;
        if (t.gradientRadius >= 0 && t.anim_border_direction=='decrease') setTimeout (function () {
            t.setGradient(t);
    ...
    setTextElementColor(t) {
        if ($(t.elText).is('.mouseover'))
            $(t.elText).removeClass('mouseover').addClass('mouseout');
        else
            $(t.elText).removeClass('mouseout').addClass('mouseover');
        /*
        if (!t.cgTheme) {
...
class naVividButton_icon_svg {
    ...
    async init (buttonHTMLid, parentObject) {
        ...
        btnCode = vui.vividButton.buttonTypes[buttonType];
        ...
        if (!vui.vividButton.settings) vui.vividButton.settings = { buttons : {} };
        vui.vividButton.settings.buttons['#'+buttonHTMLid] = {
            el : el,
            type : buttonType,
            btnCode : btnCode,
            circumstance : 'normal',
        ...
        t.settings.buttons['#'+buttonHTMLid] = vui.vividButton.settings.buttons['#'+buttonHTMLid];
                                        ...
                                        for (var animTo_stateName in stateLayer.animTo) {
                                            ...
                                            for (var i=0; i<a.animSteps; i++) {
                                                ...
                                                for (var j=0; j<stops.length; j++) {
                                                    ...
                                                    if (!v.colorSteps[j]) v.colorSteps.push(naCG.generateList_basic (cgTheme, a.animSteps));
